...
## Sustained 28.8 kbit/s qualification and deployment

Attempt `66f1104b-5c88-4b09-8ed0-1ff83b54bed1` ran the verified `dcb450f` binary as native PID13593, upstream 28800 and automatic echo acquisition. All 129 target response hashes passed on one PPP connection in 621.123 seconds, with no failed requests or retries. This covers 2 MiB of target downloads, 64 KiB of target uploads and a final result check. Windows ended with 15 CRC and one alignment error; timeout, framing, buffer and hardware-overrun counters were zero. It is a complete transfer qualification, not error-free modem operation.

Native downstream CP remained 49.333 kbit/s throughout this run. Echo acquisition selected 1428 samples at sample 96480 and B1 correlation was 0.9767. The only later S/Sbar, at native Phase4 time 631.450/631.460 seconds, accompanied hangup. No mid-call rate renegotiation was recorded. Native and Windows clocks have different origins.

The RTP capture contained 130539 packets with zero kernel drops or sequence gaps. All 32499 downstream primary payloads matched; all 32501 forwarded upstream payloads matched after eleven ATA startup packets. The TCP capture contained 4494 packets with zero kernel drops. All 129 flows included server SYNs and matched the probe kinds in order; 59 downstream segments overlapped earlier sequence ranges. Packet preservation does not establish continuous analog playout.
...
## Negotiation fix hardware comparison

The `a5f6108` CT105 binary is SHA-256 `4500a780b2d385ecfe8cf6a62ecd5eb5e000995bee7d9cb9a05e8284993a9f43`. CT105 ASan/UBSan checks passed for explicit per-call selection, E reset and retraining across all profiles and both large-constellation capability states.

At configured 31.2 kbit/s, attempt `11d967c1-a47f-4cf4-a55b-e1677ecf359f` used verified PID14365. The live log confirms peer large-constellation support and selected rate 31200. It still failed with Windows error 721 before PPP connected, with no valid upstream PPP frame. Downstream CP was 49.333 kbit/s; upstream E/B1 were detected at 4.041125/4.081125 seconds with correlation 0.9831. Echo acquired 1428 at sample 96640. Correcting Phase2/MP consistency therefore did not resolve this hardware failure.

Its capture contained 13204 packets, zero kernel drops and no RTP sequence gaps. All 3272 forwarded downstream primary payloads matched; one final server packet arrived 2.704 ms after the ATA's BYE and 0.812 ms before FreePBX's BYE toward SIPfax, and was not forwarded. This was checked against both SIP timestamps. All 3274 forwarded upstream payloads matched after twelve startup packets. Raw RX/TX recordings were retained privately.

A control call on the same new binary at configured 28.8 kbit/s, attempt `f53074bd-15cb-41f5-a300-491929c98dbe`, used verified PID14529 and logged selected rate 28800. It passed the internet probe and all 18 verified response hashes in 36.022 seconds with zero modem errors. Download took 7.455 seconds; median upstream check 1.347 seconds. Native downstream CP was 49.333 kbit/s, B1 correlation 0.9673, and echo delay 1428 at sample 96480. The 13082-packet capture had zero drops or sequence gaps, 3240 matching downstream payloads and 3242 matching upstream payloads after eleven startup packets.

Both wrappers restored `dcb450f` at 28800/auto. Runtime hash, service activity, persistent configuration, notebook idle and cleared SIPFAXRED state were verified. All trial, fixture, capture and retrieval handles are terminal; CI run 34554554552 is tracked separately. The new negotiation code has a successful 28.8 hardware control, but is not permanently deployed. The unresolved 31.2 failure remains a receiver/hardware investigation; neither rate-selection consistency nor the prior offline timing sweeps recovered it. Next, inspect the saved receiver's per-lane symbol and mapping errors rather than repeat undifferentiated dial attempts.
