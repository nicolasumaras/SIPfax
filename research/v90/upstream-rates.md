...
CT105 now retains native `8aa8c5e`, with a ten-second initial PPP grace period, causal ODP training enabled, index-2 pre-emphasis for 3000 symbols/s, both supported symbol rates offered, a 28800 upstream ceiling and automatic initial echo acquisition. The verified binary SHA256 is `cac0ea0c9f93a4ce2d4479b339047ec019a3709e499f95e44d4f8cfe5d456c7c`. Normal negotiation selected 3200/28800 and passed 129 sustained hashes over 620.072 seconds with thirteen CRC errors, no alignment errors and no application retries. The forced-3000 sustained run also passed all 129 hashes, with eight CRC errors. CI for this native commit passed. The previous `9be7827` binary/configuration and pinned source archives are retained for rollback; see [deployment provenance](deployment-provenance.md). Historical entries below describe earlier temporary deployments.
...
The ODP-assisted receiver in `8a4b760`, with index-2 pre-emphasis, completed 3000/28800 startup without fallback in two trials; a later repeat needed 26400 recovery. A sustained trial passed all 129 transfer hashes on one PPP connection over 672.717 seconds, with no application retries/failures, but with 19 CRC errors, two alignment errors and 66 downstream TCP retransmitted segments. The downstream rate remained 49.333 through an observed renegotiation. No RTP sequence gaps/capture drops occurred and forwarded primary payloads match exactly. This is sustained functional evidence, not error-free qualification; that earlier experiment was not retained. `638ed25` exposes pre-emphasis as configuration, and `2c19af6` fixes a missing dependency in the feedback sanitizer command.

The 48-frame ODP prediction candidate (`8aa8c5e`) recovers initial LCP in three failed recordings and preserves all five passing replay controls; one failed recording remains unrecovered. Independent 3000/3200 waveform tests cover both immediate PPP and 4.5 seconds of idle following detection, with predicted-label checks and CT105 ASan/UBSan passing. Two consecutive short hardware calls using the configured index-2 pre-emphasis started at 3000/28800 without fallback and each passed internet access and 18 transfer hashes with zero reported modem errors. Their RTP captures have no gaps/drops and exact forwarded primary audio. These temporary experiments restored the previous baseline after each call; see `odp-horizon-48-hardware.json` for the measured outcomes.

The 48-frame candidate then passed 129 sustained transfer checks on one PPP connection over 629.806 seconds without application retries/failures, recording eight CRC errors, no alignment errors and 35 downstream TCP retransmitted segments. It started at 3000/28800 without fallback, completed one rate renegotiation at the same downstream 49.333 kbit/s, and did not fully retrain. RTP had no gaps/drops and exact forwarded primary audio. The final S sequence near hangup did not complete another renegotiation. This is functional evidence with residual errors, not sustained error-free qualification. The previous `9be7827` binary/configuration was restored and verified active.
...
The 3000-symbol/s implementation sequence below is complete through negotiation and hardware startup; sustained quality remains under qualification:
...
### First causal-training hardware qualification (2026-09-11)

Native `8a4b760` was built in CT105 with binary SHA-256 `d70bd42a5c41dc0fbc394a129fab20f49be0522324cb223342199ec46d25abca`. The reversible normal control enabled only `SIPFAX_V90_ODP_TRAINING=1`. Attempt `167933d8-b444-4bd0-b6dd-d4d215b49455` selected 3200/28800, acquired prediction at mapping frame 41 and delivered the first PPP frame at upstream time 5.871625 seconds without fallback. Internet access and all 18 transfer hashes passed over 35.321 seconds, with zero reported modem errors. The 12969-packet capture had zero kernel drops/sequence gaps, 3214 identical downstream payloads and 3227/3216 upstream payloads matching after the eleven-packet startup suffix.

A private variant adds only the previously verified INFO1d index-2 pre-emphasis request for 3000 baud (binary SHA-256 `a8ee8c47bb9115a8fe7fc0fec496407d932a56f3d40343bc237dd256a3f55475`). With 3000 baud forced and ODP training enabled, attempt `dd3b863e-bb4d-4098-92e3-6d5656076646` selected 3000/high/28800 and acquired predictions at mapping frames 27/28. The first PPP frame arrived at 5.864375 seconds. Unlike the earlier paired trials, it did not retrain or reduce the upstream ceiling. Internet access and all 18 transfer hashes passed over 38.756 seconds with zero reported modem errors. The downstream CP remained 148000/3 bit/s. The 13631-packet capture had zero kernel drops/sequence gaps, 3380 identical downstream payloads and 3393/3382 upstream payloads matching after the eleven-packet startup suffix.

Both calls disconnected cleanly. Captures and fixtures were closed, and the production `9be7827` binary/hash and original configuration were restored and verified active. Aggregate results are in `odp-hardware-trials-2026-09-11.json`; raw recordings are retained privately for replay. This is the first successful short 3000/28800 hardware trial in this experiment sequence, not sustained qualification. Next gates are a sustained transfer run, repeatability and a reproducible way to select pre-emphasis without relying on a private source variant. The newer flat recording with no decoded ODP and optional 31200 remain unresolved.


### Reproducible pre-emphasis selection (2026-09-11)

`SIPFAX_V90_PREEMPHASIS_3000` replaces the private source patch with an explicit optional INFO1d setting. Exact values 0 through 10 select the standardized index; missing or malformed values use flat index 0. The option affects only enabled 3000-symbol profiles and is captured at startup and preserved through full retrains. The 3200 offer remains flat. The successful private index-2 build remains the binary under sustained hardware test while this configuration change is qualified separately.

The expanded independent wire test passes 1536 carrier/mode/profile cases across default, index 2 and index 10, plus invalid configuration cases. It verifies CRCs, capability masks, rejected offers, selected training/Phase4 profiles and preservation despite an environment change before retraining. Local ASan linking is unavailable because its runtime library is missing; the ordinary native checks run locally, and sanitizer qualification is deferred until the hardware call ends so CT105 is not compiled during the call.


### Sustained run and configuration repeat (2026-09-11)

The private index-2/ODP-training variant sustained attempt `ae5c1f4b-f1b3-4e1b-b837-984f33f5b987` for 672.717 seconds. All 129 transfer hashes passed with no application retries or failures on one PPP connection. Windows reported 19 CRC errors, two alignment errors and zero other errors. This took about 16% longer than the earlier 581.627-second 3000/26400 recovery run, a single-run comparison rather than a repeated benchmark. Native logs show completed rate renegotiations at Phase4 times 309.233750 and 473.194000, both retaining 148000/3 bit/s downstream, followed by a caller-requested full retrain at startup time 486.354750. The full retrain retained 3000/28800 and 148000/3 downstream, and PPP transfers continued. The final S indication occurred near hangup and is not counted as a completed renegotiation.

The 140897-packet RTP and 4510-packet TCP captures had zero kernel drops. RTP had zero sequence gaps: all 35077 forwarded downstream primary payloads match, and the 35090/35079 upstream payloads match after the eleven-packet startup suffix. The 129 TCP flows include 66 downstream retransmitted segments. Thus successful application hashes coexist with a materially imperfect link; absence of RTP gaps is not proof of analogue sample quality. Aggregate evidence is in `odp-sustained-2026-09-11.json`.

Build `2c19af6` exposes the same pre-emphasis request as configuration and fixes the feedback sanitizer's missing ODP link dependencies. CT105 binary SHA-256 is `9668bc9b1005faf19ed0f19f9fbe16429bce0a87c9533e87bf1c088d8b8e85a9`. The repaired feedback sanitizer, startup-profile sanitizer and 289-case startup guard pass ASan/UBSan on CT105. Remote CI for `8a4b760` failed only at that missing-link target; it is not reported as a passing workflow.

Repeat attempt `974a5779-dc0a-4409-a2f5-72e3d96ddc7d`, using configured index 2 and ODP training, acquired prediction at mapping frame 29 but did not start PPP at 28800. It recovered to 26400, then passed internet access and all 18 hashes over 55.389 seconds with one CRC and one alignment error. Independent demodulation of the outgoing recording verifies all 109 INFO1d bits and CRC, including the index-2 request: the option reached the wire. The 16980-packet capture had zero drops/sequence gaps. All 4211 forwarded downstream primary payloads match; one final server packet at epoch 1789112366.999411 follows the ATA BYE at 1789112366.994862 and was not forwarded. Upstream 4224/4213 payloads match after the eleven-packet startup suffix.

All calls, fixtures and captures were closed. Production was restored to the verified `9be7827` binary/configuration and checked active. The new settings remain experimental because 3000/28800 startup is not repeatable and sustained errors require investigation. The new failed startup and successful sustained recording are retained privately for comparison; the latter also permits comparison before/after the caller-requested full retrain. A separate replay of the two old 31200 failures produces zero PPP frames and zero causal-prediction uses with the option on or off, so this change does not resolve that optional profile.


### Independent downstream PCM audit (2026-09-11)

The sustained `8aa8c5e` recording was inversely mapped using its received CP parameters. Raw intervals 24–599 and 612–650 seconds contain 2045 and 146 FCS-valid PPP frames respectively, with no bad FCS frames, invalid constellation samples or out-of-range magnitude mappings at the selected alignment. The intervals exclude training and renegotiation; this does not prove that every possible frame was extracted. All 4,904,000 samples in these intervals match decoded primary RTP sent by FreePBX toward the ATA exactly. A copied recording with one sign-inverted sample produces a bad PPP checksum and a failing `--require-clean` result.

`audit_pcm.py` provides the metadata-only inverse mapping diagnostic for stable mu-law/Sr=1 intervals. The aggregate is in `downstream-pcm-audit-2026-09-11.json`; raw data remains private. These observations support investigating the path after PCM generation without claiming to distinguish ATA DAC behavior from analogue reception. The preceding sustained call's rate renegotiation selected slightly different PCM Ucode sets while keeping rate, gain and spectral filter fixed. Packet timing shows about 0.16 ppm relative clock difference between directions; it does not measure the DAC clock directly.


### Normal-negotiation sustained control and retained deployment

Attempt `361dd160-ee10-4dc9-9830-2cbd7f2ee8f6` selected 3200/28800, acquired ODP prediction at frame 45 and held 49.333 kbit/s downstream. It passed all 129 checks over 620.072 seconds with no application retries/failures, thirteen CRC errors, no alignment errors and 71 downstream TCP retransmitted segments. No full retrain or completed rate renegotiation occurred; the S sequence near hangup did not complete a renegotiation. RTP had no gaps/capture drops, all 32446 downstream primary payloads matched, and 32459/32448 upstream payloads matched after eleven startup omissions.

The ATA's post-call counters show one underflow, 34 network-lost packets, 85 ms average playout and an 890 ms maximum interarrival, while the outgoing PBX capture is continuous. These counters have no event timestamps; they do not establish when the event happened or that it caused the later CRC errors. Normal and forced-3000 calls both show residual downstream errors, so this is not confined to the 3000 receiver.

With sustained functional tests for both required symbol rates and CI passing, `8aa8c5e` was retained on CT105 using the normal-negotiation configuration. This is a development deployment with explicit remaining quality limitations, not error-free qualification or full conformance. The previous binary/configuration is preserved in `/opt/sipfax/releases/` for rollback.
