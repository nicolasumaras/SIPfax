...
return n;}
static void status(void *opaque,int code){struct peer*p=opaque;if(code==SIG_STATUS_LINK_CONNECTED)p->up++;if(code==SIG_STATUS_LINK_DISCONNECTED)p->down++;if(code==SIG_STATUS_LINK_ERROR)p->errors++;}
static void sink(void *opaque,const uint8_t *in,int n){struct peer*p=opaque;if(n<0){status(p,n);return;}if(p->v->lapm.local_busy)busy_violation++;for(int i=0;i<n;i++){if(p->received>=TOTAL || in[i]!=data(p->side^1,p->received)){fprintf(stderr,"Mismatch at sample %u side %u byte %u\n",sample,p->side,p->received);exit(2);}p->received++;}}
