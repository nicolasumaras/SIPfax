...
''')
    subprocess.run(['gcc','-O2','-Wall','-Werror','-shared','-fPIC',
                    ...
                    str(root/'vendor/linmodem/v90trellis.c'),str(root/'vendor/linmodem/v90qam8.c'),str(root/'vendor/linmodem/v90equalizer.c'),str(root/'vendor/linmodem/v90shell.c'),str(root/'vendor/linmodem/v90mapping.c'),str(root/'vendor/linmodem/v42detect.c'),'-lm','-o',str(d/'b1.so')],check=True)
