...
''')
    ...
    def train(s,x,y,half=True):
        return getattr(lib,'v90_equalizer_train_half' if half else 'v90_equalizer_train')(s,*[np.ascontiguousarray(z) for z in [x.real,x.imag,y.real,y.imag]])
    def run(s,x):
        out=[]
    ...
    for channel in [np.array([.2-.08j,1,-.15j]),np.array([.12j,.1,1,-.14-.05j,.08])]:
        ...
        s=lib.create();assert train(s,distorted[:256],desired[:128])
        corrected=run(s,distorted)
        before=np.mean(abs(distorted[1::2][128:-7]-desired[128:-7])**2)
        after=np.mean(abs(corrected[128:]-desired[128:-7])**2)
        assert after<.02*before,(before,after)
        old=lib.clone(s)
        bad=desired[:128].copy();bad[87:]*=-1
        assert not train(s,distorted[:256],bad) and lib.same(s,old)
        assert not train(s,distorted[:128],desired[:128],False) and lib.same(s,old)
        for invalid in [float('nan'),float('inf'),1e100]:
            broken=distorted[:256].copy();broken[255]=invalid
            assert not train(s,broken,desired[:128]) and lib.same(s,old)
            r=C.c_double(999);i=C.c_double(999)
    ...
    assert not train(s,samples[:128],desired[:128]) and lib.same(s,old)
    lib.destroy(old);lib.destroy(s)
print('PASS: half-symbol delay, held-out channel recovery, mode checks and unchanged state on rejected input/updates')
