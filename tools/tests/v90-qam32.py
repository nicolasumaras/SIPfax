...
m,k=(14,30) if '--16800' in sys.argv else (8,24)
frame_bits=k+12
point_count=4*m
...
points=[z*(-1j)**q for z in [1+1j,-3+1j,1-3j,-3-3j,1+5j,5+1j,-3+5j,5-3j,5+5j,-7+1j,1-7j,-7-3j,-3-7j,-7+5j][:m] for q in range(4)]
def subset(z):
...
''')
    if m==14:w.write_text(w.read_text().replace('V90Qam32Frames','V90Qam56Frames').replace('v90_qam32_frames_init','v90_qam56_frames_init').replace('s,14400','s,16800'))
    ...
    for index in indices:
        shell=oracle[index];bits=[(index>>i)&1 for i in range(k)]+[rng.randrange(2) for _ in range(12)];labels=[]
        for p in range(4):
            a=(previous+bits[k+1+3*p]+2*bits[k+2+3*p])%4
            b=(a+2*bits[k+3*p]+rng.randrange(2))%4;previous=a
    ...
    if m==14:
        ...
        assert not frame_fn(f,(C.c_uint8*8)(*[4*r for r in oracle[1<<k]]),out)
    ...
    for initial in range(16):
        ...
        for n in range(1600):
            ...
            a+=4*rng.randrange(m);b+=4*rng.randrange(m);pairs.append((a,b));invbits.append(inv)
