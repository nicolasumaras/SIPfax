...
''')
    ...
    subprocess.run(['gcc','-O2','-Wall','-Werror','-shared','-fPIC','-I'+str(src),str(wrapper),*[str(src/f) for f in ['v90pcm.c','v90training.c','v90cp.c','v90dil.c','v90upstream.c','v90trellis.c','v90qam8.c','v90equalizer.c','v90shell.c','v90mapping.c','v42detect.c','v90odp.c']],'-lm','-o',str(so)],check=True)
    ...
    for setting,frames,rate in [(setting,frames,rate) for setting,frames in [(None,340),('0',0),('1',1),('255',340),('1500',2000),('2000',2666),('-1',340),('2001',340),('junk',340),('999999999999999999999',340)] for rate in [None,'7200','9600','12000','14400','16800','19200','21600','24000','26400','28800','31200','invalid']]:
        ...
        try:
            ...
            assert sum(mp[24+k]<<k for k in range(4))==(13 if rate=='31200' else 12 if rate=='28800' else 11 if rate=='26400' else 10 if rate=='24000' else 9 if rate=='21600' else 8 if rate=='19200' else 7 if rate=='16800' else 6 if rate=='14400' else 5 if rate=='12000' else 4 if rate=='9600' else 3 if rate=='7200' else 2)
            assert mp[36:50]==[int(k==(11 if rate=='31200' else 10 if rate=='28800' else 9 if rate=='26400' else 8 if rate=='24000' else 7 if rate=='21600' else 6 if rate=='19200' else 5 if rate=='16800' else 4 if rate=='14400' else 3 if rate=='12000' else 2 if rate=='9600' else 1 if rate=='7200' else 0)) for k in range(14)]
            crc=0xffff
