...
''')
    subprocess.run(['gcc','-shared','-fPIC','-O2','-Wall','-Werror','-I'+str(root/'vendor/linmodem'),str(w),str(root/'vendor/linmodem/v90upstream.c'),str(root/'vendor/linmodem/v90trellis.c'),'-lm','-o',str(so)],check=True)
