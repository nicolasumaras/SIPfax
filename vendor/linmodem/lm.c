...
void sm_process(struct sm_state *sm, s16 *output, s16 *input, int nb_samples)
{
    ...
    case SM_V23:
        ...
        /* V34 handling (both calling & receive) */
    case SM_V34:
        {
            int ret;
        ...
        /* V90 handling (server=encode downstream, client=decode) */
    case SM_V90:
        {
            int ret;
            ret = V90_process(&sm->u.v90_state, output, input, nb_samples);
            if (ret || sm->hangup_request)
                sm->state = SM_GO_ONHOOK;
        }
        break;
...
void lm_init(struct sm_state *sm, struct sm_hw_info *hw, const char *name)
{
    ...
    /* SIPFAX: optionally cap the advertised modulations (e.g. force V.34 by
       dropping V.90 from the V.8 menu) without a rebuild */
    {
...
int main(int argc, char **argv)
{
    ...
    {
        char *cf = getenv("SIPFAX_CMA_FILE");
        if (cf) { extern void V34_cma_decode_file(const char *); V34_cma_decode_file(cf); return 0; }
    }
    {
    ...
    }
    {
        ...
        if (df) {
            ...
            char *col = strchr(df, ':');
            int calling = 1;
            if (col) { *col = 0; calling = atoi(col + 1); }
            V34_decode_file(df, calling);
            return 0;
        }
    }
    ...
    for(;;) {
        c = getopt(argc, argv, "hvstrac:d:m:P");
        if (c == -1) break;
        switch(c) {
        case 'v':
          lm_debug++;
