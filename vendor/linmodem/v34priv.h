...
#define P4_RING_MASK (P4_RING_SZ - 1)
    ...
    double ted_ema;
    int jvar_defer;    /* SIPFAX: hold the Phase-4 switch until the J vote lands */
    ...
    /* SIPFAX: Y0 for each decoded 4D symbol, taken from the Viterbi survivor at the
       traceback point and buffered alongside rx_mapping_frame, so decode_mapping_frame can
...
typedef struct V34State {
    ...
    double p3x1; double p3h[63];   /* SIPFAX: sized for the 63-tap index-2 pre-emphasis */
