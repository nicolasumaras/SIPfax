...
int V90_process(struct V90State *s, s16 *output, s16 *input, int nb_samples)
{
    ...
    u8 data[64];
    if (s->calling) {
        /* client: recover downstream data; upstream V.34 TODO -> silence out */
        for (i = 0; i < nb_samples; i++) {
            s->framebuf[s->fpos++] = input[i];
            if (s->fpos == 6) {
                v90_decode_mapping_frame(&s->dec, data, s->framebuf);
                for (j = 0; j < s->n; j++)
                    if (s->put_bit) s->put_bit(s->opaque, data[j]);
                s->fpos = 0;
            }
            output[i] = 0;
        }
