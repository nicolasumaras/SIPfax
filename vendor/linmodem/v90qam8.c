...
int v90_qam_b1_init_profile(V90Qam8B1 *s,unsigned rate,unsigned symbol_rate)
{
    ...
    memset(s,0,sizeof(*s));
    V90Mapping mapping;
    if(!v90_mapping_init(&mapping,rate,symbol_rate))return 0;
    unsigned length=v90_mapping_b1(&mapping,s->labels,V90_QAM8_B1_SYMBOLS);
    if(!length)return 0;
    s->m=mapping.m;s->k=mapping.k;s->q=mapping.q;s->length=length;
...
{
    /* Preserve the legacy API's exclusion of the separate 4800 receiver. */
    if(!s)return 0;
    if(rate==4800){memset(s,0,sizeof(*s));return 0;}
    return v90_qam_b1_init_profile(s,rate,3200);
}
...
{
    if(!s)return 0;
    if((s->length!=120 && s->length!=128) || !s->m || !isfinite(re)||!isfinite(im)||fabs(re)>1e100||fabs(im)>1e100) {
        s->position=s->count=0;return 0;
    }
    s->re[s->position]=re;s->im[s->position]=im;
    s->position=(s->position+1)%s->length;
    if(s->count<s->length)++s->count;
    if(s->count<s->length)return 0;
    double cr=0,ci=0,energy=0;
    for(unsigned i=0;i<s->length;++i) {
        unsigned j=(s->position+i)%s->length;
