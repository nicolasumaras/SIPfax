        ...
        case GI_PARAM_NEGOTIATION:
            while (group_len > 0)
            {
                ...
                case PI_TX_INFO_MAXSIZE:
                    ...
                    param_val >>= 3;
                    config.v42_rx_n401 =
                    s->rx_n401 = set_param(ss->config.v42_rx_n401, param_val, V42_DEFAULT_N_401);
                    break;
                case PI_RX_INFO_MAXSIZE:
                    param_val = pack_value(buf, param_len);
                    param_val >>= 3;
                    config.v42_tx_n401 =
                    s->tx_n401 = set_param(ss->config.v42_tx_n401, param_val, V42_DEFAULT_N_401);
                    break;
                case PI_TX_WINDOW_SIZE:
                    param_val = pack_value(buf, param_len);
                    config.v42_rx_window_size_k =
                    s->rx_window_size_k = set_param(ss->config.v42_rx_window_size_k, param_val, V42_DEFAULT_WINDOW_SIZE_K);
                    break;
                case PI_RX_WINDOW_SIZE:
                    param_val = pack_value(buf, param_len);
                    config.v42_tx_window_size_k =
                    s->tx_window_size_k = set_param(ss->config.v42_tx_window_size_k, param_val, V42_DEFAULT_WINDOW_SIZE_K);
                    break;
                default:
                    break;
                }
...
static void transmit_xid(v42_state_t *ss, uint8_t addr)
{
    ...
    *buf++ = 2;
    put_net_unaligned_uint16(buf, (addr == s->rsp_addr ? s->tx_n401 : ss->config.v42_tx_n401) << 3);
    buf += 2;

    /* Send the maximum as a number of bits, rather than octets */
    *buf++ = PI_RX_INFO_MAXSIZE;
    *buf++ = 2;
    put_net_unaligned_uint16(buf, (addr == s->rsp_addr ? s->rx_n401 : ss->config.v42_rx_n401) << 3);
    buf += 2;

    *buf++ = PI_TX_WINDOW_SIZE;
    *buf++ = 1;
    *buf++ = addr == s->rsp_addr ? s->tx_window_size_k : ss->config.v42_tx_window_size_k;

    *buf++ = PI_RX_WINDOW_SIZE;
    *buf++ = 1;
    *buf++ = addr == s->rsp_addr ? s->rx_window_size_k : ss->config.v42_rx_window_size_k;
...
static void tx_information_rr_rnr_response(v42_state_t *ss, const uint8_t *frame, int len)
{
    ...
    if ((frame[2] & 0x1)  ||  !tx_information_frame(ss))
        tx_supervisory_frame(s, frame[0], (s->local_busy)  ?  LAPM_S_RNR  :  LAPM_S_RR, frame[2] & 1);
}
/*- End of function --------------------------------------------------------*/
...
static void initialize_lapm_parameters(v42_state_t *ss)
{
...
static void negotiation_rx_bit(v42_state_t *s, int new_bit)
{
    ...
    /*endif*/
    new_bit &= 1;
    s->neg.rxstream = ((s->neg.rxstream & 0x1FF) << 1) | new_bit;
